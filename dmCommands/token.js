...
module.exports = {
    ...
    async execute (client, message, cmd, args, Discord) {
        ...
        if (args.length == 2) {
            ...
            pool.query("SELECT * FROM guilds WHERE id = ?", [args[0]], function(error, results) {
                ...
                pteroClient.getAccountDetails().then(res => {
                    pool.query('INSERT INTO users SET ?', {guildId: args[0], userId: authorId, token: args[1]}, function (error) {
                        if (error) throw error;
